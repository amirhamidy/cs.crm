...
export default function CustomersPage() {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const [customersRes, casesRes, tasksRes, employeesRes] = await Promise.all([
                axiosInstance.get<ListResponse<Customer>>("/customers/api/v1/customers/"),
                axiosInstance.get<ListResponse<CaseItem>>("/tasks/api/v1/cases/"),
            ...
            const allCustomers = extractList<Customer>(customersRes.data);
            const allCases = extractList<CaseItem>(casesRes.data);
            const allTasks = extractList<TaskItem>(tasksRes.data);
            ...
            const detailedCases = await Promise.all(
                allCases.map(async (c) => {
                    try {
                        const detailRes = await axiosInstance.get(`/tasks/api/v1/cases/${c.id}/`);
                        return { ...c, ...detailRes.data };
                    } catch {
                        return c;
                    }
                })
            ...
            const userTaskCaseIds = new Set(
                userTasks
                    .map((t: any) => {
                        const raw = t.case ?? t.case_id ?? t.caseId;
                        return typeof raw === "object" ? Number(raw?.id) : Number(raw);
                    })
            ...
            detailedCases.forEach((c: any) => {
                ...
                const custId = extractCaseCustomerId(c);
                if (!custId) return;

                const isAssignedDirectly =
                    empId &&
                    (parseAssignedEmployees(c.assigned_employee).includes(empId) ||
                        parseAssignedEmployees(c.employees).includes(empId) ||
                        (userId && Number(c.created_by ?? c.creator ?? c.user) === Number(userId)));

                const hasTaskInCase = userTaskCaseIds.has(cId);

                if (!empId || isAssignedDirectly || hasTaskInCase) {
                    allowedCaseCustomerIds.add(custId);
                    activeCaseCustomerIds.add(custId);
                }
            });
