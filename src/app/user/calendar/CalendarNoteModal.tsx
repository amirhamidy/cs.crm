...
export default function CalendarNoteModal({
    ...
    const dayEvents = useMemo(() => {
        if (!selectedKey) return [];
        return events.filter((ev) => ev.start?.split("T")[0] === selectedKey);
    }, [events, selectedKey]);
            ...
            {isOpen && selectedDate && (
                ...
                >
                    <motion.div
                        ...
                        dir="rtl"
                        className="w-full max-w-2xl rounded-[2rem] border border-gray-100 dark:border-white/[0.06] bg-white/95 dark:bg-[#0f172a]/95 shadow-[0_30px_80px_-30px_rgba(0,0,0,0.35)] backdrop-blur-2xl"
                        ...
                        <div className="space-y-5 p-6">
                            {dayEvents.length > 0 && (
                                <div className="flex flex-col gap-2">
                                    <p className="text-[10.5px] font-bold text-gray-400">رویدادهای این روز</p>
                                    {dayEvents.map((ev) => {
                                        ...
                                        return (
                                            <div key={ev.id} className={`flex items-start gap-2.5 rounded-2xl ${s.bg} px-3 py-2.5`}>
