...
import "swiper/css";
import { Building2, Users, ArrowRight, Loader, AlertCircle, ListChecks } from "lucide-react";
...
type DeleteTarget = { type: "stage"; stage: Stage } | { type: "employee"; employee: Employee } | null;

const stageColors = ["#6366f1", "#8b5cf6", "#ec4899", "#f59e0b", "#10b981", "#3b82f6", "#ef4444", "#14b8a6"];
...
export default function DepartmentDetailPage() {
    ...
    const department = useMemo(() => departments.find((department) => String(department.id) === String(id)) ?? null, [departments, id]);
    ...
    useEffect(() => {
        let mounted = true;
        const init = async () => {
            if (departments.length === 0) await fetchAll();
    ...
    const departmentTasks = useMemo(() => {
        if (!department?.stages?.length) return [];
        const stageIds = new Set(department.stages.map((stage) => String(stage.id)));
        return tasks.filter((task) => {
            const stageId = getRelationId(task.current_step);
            const status = String(task.status ?? "").toLowerCase();
            if (!stageId || !stageIds.has(stageId)) return false;
            return !["completed", "cancelled", "sold"].includes(status);
        });
    }, [tasks, department]);
        ...
        try {
            if (deleteTarget.type === "stage") {
                ...
                if (targetId !== undefined && targetId !== null && !String(targetId).startsWith("temp-")) {
                    await deleteStage(id, String(targetId));
                }
            } else {
                const targetEmpId = deleteTarget.employee.id;
                if (targetEmpId !== undefined && targetEmpId !== null) await removeEmployee(id, String(targetEmpId));
            }
        } finally {
    ...
    const deleteModalMeta = useMemo(() => {
        ...
        if (deleteTarget.type === "stage") return { title: `حذف فرآیند "${deleteTarget.stage.name}"`, description: "این فرآیند از دپارتمان حذف می‌شود." };
        return { title: `حذف عضو "${deleteTarget.employee.name}"`, description: "این عضو از دپارتمان حذف می‌شود." };
    }, [deleteTarget]);
    ...
    return (
        <div className="flex flex-col gap-4" dir="rtl">
            <DeleteModal open={!!deleteTarget} title={deleteModalMeta.title} description={deleteModalMeta.description} loading={deleteLoading} onConfirm={handleConfirmDelete} onCancel={() => setDeleteTarget(null)} />
            ...
            <div className="flex items-center justify-between gap-4">
                <div className="flex min-w-0 items-center gap-2.5">
                    ...
                    <div className="min-w-0">
                        <h3 className="truncate text-[13.5px] font-extrabold text-gray-900 dark:text-white">{department.name}</h3>
                        <div className="mt-0.5 flex items-center gap-3 text-[11px] text-gray-400 dark:text-gray-600">
                            <span className="flex items-center gap-1"><Users size={9} />{department.employees?.length ?? 0} عضو</span>
                            <span className="flex items-center gap-1"><ListChecks size={9} />{departmentTasks.length} تسک فعال</span>
                        </div>
                    </div>
                </div>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22 }} className="flex flex-col gap-3">
                <EmployeesPanel department={department} onAddEmployee={() => setAddEmployeeOpen(true)} onDeleteEmployee={(employee) => setDeleteTarget({ type: "employee", employee })} />

                <div className="relative">
                    <AnimatePresence>
                        {stageRefreshing && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-10 flex items-center justify-center rounded-[1.5rem] bg-white/70 backdrop-blur-sm dark:bg-black/50">
                                <Loader size={19} className="animate-spin text-indigo-500" />
                            </motion.div>
                        )}
                    </AnimatePresence>
                    ...
                    <StagesPanel
                        ...
                        onReorder={(stages) =>
                            useDepartmentStore.setState((state) => ({
                                departments: state.departments.map((departmentItem) => String(departmentItem.id) === String(id) ? { ...departmentItem, stages } : departmentItem),
                            }))
                        }
                    />
                </div>
