...
export default function SentTaskCard({
    ...
    const [timeError, setTimeError] =
        useState<string | null>(null);
    const [stepDeadline, setStepDeadline] =
        useState<StepDeadline>({
            started_at:
    ...
    const assignedEmployees: EmployeeRef[] =
    ...
    const status: InternalTaskStatus =
        (task.status as InternalTaskStatus) ||
        "in_progress";
    ...
    async function handleDelete() {
        ...
        try {
            ...
            setDeleteError(
                "خطا در حذف تیکت. دوباره تلاش کنید.",
            );
        } finally {
            setDeleting(false);
        }
                ...
                style={{
                    border: accent
                        ...
                        : isDark
                            ...
                            : "1px solid rgba(0,0,0,0.06)",
                    background: accent
                        ? `color-mix(in srgb, ${accent} ${urgency ===
                            "overdue"
                            ? 7
                            : urgency ===
                                "critical"
