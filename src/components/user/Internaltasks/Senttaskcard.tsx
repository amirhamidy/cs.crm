...
const AVATAR_GRADIENTS = [
    ...
    ["#06b6d4", "#6366f1"], ["#f59e0b", "#ef4444"], ["#10b981", "#3b82f6"],
    ["#f472b6", "#ec4899"], ["#8b5cf6", "#f59e0b"], ["#3b82f6", "#06b6d4"],
    ["#ef4444", "#f59e0b"],
];

export const STATUS_CONFIG: Record<InternalTaskStatus, { label: string; className: string }> = {
    in_progress: { label: "در حال انجام", className: "border-amber-500/20 bg-amber-500/10 text-amber-400" },
    completed: { label: "انجام شده", className: "border-blue-500/20 bg-blue-500/10 text-blue-400" },
    cancelled: { label: "لغو شده", className: "border-red-500/20 bg-red-500/10 text-red-400" },
};
...
const URGENCY_ACCENT = { overdue: "#ef4444", critical: "#f97316", soon: "#eab308" };
const URGENCY_LABEL = { overdue: "منقضی شده", critical: "فوری", soon: "امروز" };
...
function EmployeeChip({ employee, isDark }: { employee: EmployeeRef; isDark: boolean }) {
    const name = employee.full_name?.trim() || (employee as any).username?.trim();
    if (!name) return null;
    const gradient = getGradient(Number(employee.id) || 0);

    return (
        <div className="flex items-center gap-1.5 rounded-full py-0.5 pl-2 pr-0.5" style={{
...
export default function SentTaskCard({
    ...
    const assignedEmployees: EmployeeRef[] = Array.isArray(task.assigned_to)
        ? task.assigned_to.filter((employee: any) =>
            Boolean(employee?.full_name?.trim() || employee?.username?.trim()))
        : [];
    ...
    async function handleTimeSubmit(startedAt: string, deadline: string) {
        ...
        try {
            ...
            const value = data?.data ?? data;
            const updatedStartedAt = value?.started_at ?? startedAt;
            const updatedDeadline = value?.deadline ?? deadline;

            setStepDeadline({ started_at: updatedStartedAt, deadline: updatedDeadline });
            onUpdated?.({ ...task, started_at: updatedStartedAt, deadline: updatedDeadline });
    ...
    return (
        <>
            <motion.div
                ...
                className="relative flex min-h-[130px] flex-col gap-3 overflow-hidden rounded-[2rem] p-4"
                style={{
                    border: accent
            ...
            >
                ...
                <svg className="pointer-events-none absolute inset-0 h-full w-full">
                    <defs>
                        <linearGradient id={`borderGrad-internal-${task.id}`} x1="100%" y1="100%" x2="0%" y2="0%">
                            <stop offset="0%" stopColor="#6366f1" />
                            <stop offset="100%" stopColor="#8b5cf6" />
                        </linearGradient>
                    </defs>
                    <motion.rect
                        x="1" y="1" width="calc(100% - 2px)" height="calc(100% - 2px)" rx="30" ry="30"
                        fill="none" stroke={`url(#borderGrad-internal-${task.id})`} strokeWidth="1.5"
                        pathLength="1"
                ...
                <div className="flex items-start justify-between gap-3">
                        ...
                        {accent && (
                            ...
                            >
                                <motion.span
                                    ...
                                    animate={urgency !== "soon" ? { opacity: [1, .3, 1] } : undefined}
                    ...
                    <div className="flex items-center gap-1.5">
                        <button
                            ...
                            style={{
                                background: isDark ? "rgba(99,102,241,.1)" : "rgba(99,102,241,.07)",
                        ...
                        {onDelete && (
                            <button
                                ...
                                style={{
                                    background: isDark ? "rgba(239,68,68,.1)" : "rgba(239,68,68,.07)",
                                    color: "#ef4444",
                                }}
                                title="حذف تسک"
                            >
                                {effectiveDeleting ? <Loader2 size={11} className="animate-spin" /> : <Trash2 size={11} />}
                            </button>
                        )}
                    </div>
                </div>
                ...
                <div className="mt-auto flex flex-wrap items-center gap-2 border-t pt-2.5" style={{ borderColor: isDark ? "rgba(255,255,255,.05)" : "rgba(0,0,0,.05)" }}>
                    {assignedEmployees.length
                        ? assignedEmployees.map((emp) => (
                            <EmployeeChip key={`${emp.id}-${(emp as any).username ?? ""}`} employee={emp} isDark={isDark} />
                        ))
                        : (
                            <div className="flex items-center gap-1.5 rounded-full py-0.5 pl-2 pr-0.5" style={{
                                ...
                                <span className="flex h-5 w-5 items-center justify-center rounded-full text-white" style={{ background: "linear-gradient(135deg,#6366f1,#8b5cf6)" }}>
                                    <UserRound size={11} />
                                </span>
                                <span className="text-[10.5px] font-bold" style={{ color: isDark ? "#cbd5e1" : "#475569" }}>
                                    بدون مسئول
                ...
                <div className="flex flex-col gap-1.5 rounded-[1.25rem] px-3 py-2.5" style={{
                    background: accent
                        ...
                        : isDark ? "rgba(99,102,241,.06)" : "rgba(99,102,241,.05)",
                    border: accent
                        ...
                        <>
                            {startedAtLabel && (
                                <div className="flex items-center gap-2 text-[11px] font-bold" style={{ color: accent ?? (isDark ? "#a5b4fc" : "#6366f1") }}>
                                    <CalendarDays size={13} /> <span>شروع: {startedAtLabel}</span>
                                </div>
                            )}
                            {deadlineLabel && (
                                <div className="flex items-center gap-2 text-[11px] font-bold" style={{ color: accent ?? "#ef4444" }}>
                                    <Clock size={13} /> <span>مهلت: {deadlineLabel}</span>
                                </div>
                            )}
                        </>
                    ) : (
                        <div className="flex items-center gap-2 text-[10.5px] font-semibold" style={{ color: isDark ? "#64748b" : "#94a3b8" }}>
                            <Clock size={12} /> زمان‌بندی تعیین نشده
                        </div>
                    )}
                </div>
            </motion.div>
                ...
                {showConfirm && (
                    ...
                    >
                        <motion.div
                            ...
                            dir="rtl"
                            className="flex w-full max-w-md flex-col overflow-hidden rounded-[2rem] border border-gray-100 bg-white shadow-sm dark:border-white/[.06] dark:bg-[#0f172a]"
                        >
                            <div className="flex items-center justify-between px-8 pb-6 pt-8">
                                <div className="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        <h3 className="text-[14px] font-extrabold text-gray-900 dark:text-white">حذف تیکت</h3>
                                        <p className="mt-0.5 text-[11px] text-gray-400">این عملیات قابل بازگشت نیست</p>
                                    </div>
                                </div>
                                <button type="button" onClick={closeConfirm} disabled={deleting} className="flex h-8 w-8 items-center justify-center rounded-xl bg-gray-100 text-gray-400 dark:bg-white/[.05]">
                                    <X size={15} />
                                </button>
                            </div>
                            ...
                            <div className="px-8 pb-2">
                                <p className="text-[12.5px] font-semibold leading-6 text-gray-500 dark:text-gray-400">
                                    تیکت <span className="font-extrabold text-gray-900 dark:text-white">{task.title}</span> برای همیشه حذف خواهد شد.
                                </p>
                                {deleteError && (
                                    <div className="mt-4 flex items-start gap-2.5 rounded-2xl bg-red-50 px-3.5 py-3 dark:bg-red-500/10">
                                        <ClipboardX size={14} className="mt-0.5 text-red-500" />
                                        <p className="text-[11.5px] font-semibold leading-5 text-red-500">{deleteError}</p>
                                    </div>
                                )}
                            </div>

                            <div className="flex items-center gap-2 px-8 pb-8 pt-5">
                                <button type="button" onClick={closeConfirm} disabled={deleting} className="h-11 flex-1 rounded-full bg-gray-100 text-[13px] font-bold text-gray-600 dark:bg-white/[.05] dark:text-gray-300">
                                    انصراف
                                </button>
                                <button type="button" onClick={handleDelete} disabled={deleting} className="flex h-11 flex-1 items-center justify-center gap-2 rounded-full bg-red-600 text-[13px] font-bold text-white">
