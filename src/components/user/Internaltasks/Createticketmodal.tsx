...
export default function CreateTicketModal({
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            const usersRequest = api.get<UserListItem[]>(
                "/accounts/api/v1/user/list/",
            );

            const employeesRequest = employeesProp
                ? Promise.resolve({ data: employeesProp })
                : api.get<EmployeeListItem[]>(
                      "/accounts/api/v1/employee/list/",
                  );

            Promise.all([usersRequest, employeesRequest])
                .then(([userRes, empRes]) => {
    ...
    const recipients = useMemo<Recipient[]>(() => {
        const usersByUsername = new Map<string, UserListItem>();
        ...
        const employeeUsernames = new Set(
            employees
                .map((emp) => emp.username?.trim().toLowerCase())
                .filter((username): username is string => Boolean(username)),
        );

        const employeeRecipients: Recipient[] = employees
            .map((emp) => {
                const username = emp.username?.trim().toLowerCase() || "";
                const matchedUser = usersByUsername.get(username);
        ...
        const adminRecipients: Recipient[] = users
            .filter((user) => {
                const username = user.username?.trim().toLowerCase() || "";
                return user.type === 1 && !employeeUsernames.has(username);
            })
            .map((user) => ({
                id: user.id,
                label: user.username,
        ...
        return [...employeeRecipients, ...adminRecipients];
    }, [users, employees]);
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (!title.trim() || selected.length === 0) {
            setError("عنوان و حداقل یک کاربر را انتخاب کن");
            return;
        }
        if (!userId) {
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="mt-0.5 text-[11px] text-gray-400 dark:text-gray-500">
                                        تیکت را برای خود یا همکارانت ارسال کن
                                    </p>
                                </div>
                            </div>
                            <button
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {recipients.map((recipient) => {
                                        const isActive = selected.includes(recipient.id);
                                        const displayName = recipient.label;
                                        return (
                                            <button
                                                key={recipient.id}
                                                type="button"
