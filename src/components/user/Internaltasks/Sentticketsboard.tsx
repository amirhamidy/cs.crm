...
function normalizeTask(item: unknown): InternalTask | null {
    ...
    const task = item as Partial<InternalTask>, id = Number(task.id);
...
export default function SentTicketsBoard() {
    ...
    const [tasks, setTasks] = useState<InternalTask[]>([]), [employees, setEmployees] = useState<EmployeeListItem[]>([]);
    const [loading, setLoading] = useState(true), [error, setError] = useState<string | null>(null);
    const [createOpen, setCreateOpen] = useState(false), [deleteLoadingId, setDeleteLoadingId] = useState<number | null>(null);
    ...
    const loadTasks = async () => {
        try {
            ...
            setTasks([]); setError("دریافت تیکت‌ها با خطا مواجه شد.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleUpdated = (updatedTask: InternalTask) => {
        ...
        setTasks(previous => previous.map(task => task.id === normalized.id ? { ...task, ...normalized, created_at: task.created_at } : task));
    };
