...
import { matchesSearch, getStockProductName, formatNumber } from "@/utils/warehouseEmployee";
    ...
    const filtered = useMemo(
        () =>
            stockInfos.filter((stock) => {
                ...
                return matchesSearch([data.id, data.code, data.unit, data.unit_name, getStockProductName(stock)], search);
            }),
        [stockInfos, search]
    );
            ...
            {filtered.length ? (
                <div className="grid gap-3 sm:grid-cols-2">
                    {filtered.map((stock, index) => {
                        ...
                        const isCritical = current <= (Number(data.minimum_stock ?? data.min_quantity ?? 0));
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <div className="mt-4 h-2 overflow-hidden rounded-full" style={{ background: isDark ? "rgba(255,255,255,0.06)" : "rgba(15,23,42,0.06)" }}>
                                            <div
                                                className={`h-full rounded-full transition-all ${isCritical ? "bg-red-400" : "bg-indigo-400"}`}
                ...
                >
                    <Boxes className="mx-auto h-9 w-9" style={{ color: mutedText }} />
