...
export default function WarehouseEmployeeTransactionCard({
    ...
    const isInitial = transaction.transaction_type === "initial";
    ...
    return (
        ...
        >
            <svg className="pointer-events-none absolute inset-0 h-full w-full">
                <defs>
                    <linearGradient id={`tx-border-${transaction.id}`} x1="100%" y1="100%" x2="0%" y2="0%">
            ...
            <div className="relative z-[1] flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl" style={{ background: accentBg }}>
                        {isInitial ? (
                            <ReceiptIcon />
                        ) : isIn ? (
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 flex items-center gap-2">
                            ...
                            >
                                {isInitial ? "موجودی اول دوره" : transaction.transaction_type_display || "تراکنش"}
                            </span>
                            {transaction.stock_out_reason_display && !isInitial && (
                ...
                <div className="shrink-0 text-left">
                    <p className="text-[16px] font-black tracking-tight" style={{ color: accentColor }}>
                        {isInitial ? "—" : `${isIn ? "+" : "-"}${formatNumber(quantityChanged)}`}
                    </p>
            ...
            <div className="relative z-[1] mt-4 flex items-center justify-center gap-3 rounded-2xl bg-gray-50 px-3 py-3 dark:bg-white/[0.025]">
                <div className="flex flex-col items-center">
                    <span className="text-[9px] font-bold text-gray-400 dark:text-white/40">موجودی قبل</span>
                    <span className="mt-0.5 text-[13px] font-black text-gray-700 dark:text-gray-200">{formatNumber(quantityBefore)}</span>
                </div>
                ...
                <div className="flex flex-col items-center">
                    <span className="text-[9px] font-bold text-gray-400 dark:text-white/40">موجودی بعد</span>
                    <span className="mt-0.5 text-[13px] font-black" style={{ color: accentColor }}>{formatNumber(quantityAfter)}</span>
                </div>
                <div className="mx-1 h-6 w-px bg-gray-200 dark:bg-white/10" />
                <div className="flex flex-col items-center">
                    <span className="text-[9px] font-bold text-gray-400 dark:text-white/40">حداقل</span>
                    <span className="mt-0.5 text-[12px] font-black text-amber-600 dark:text-amber-400">{formatNumber(transaction.minimum_stock)}</span>
                </div>
                <div className="flex flex-col items-center">
                    <span className="text-[9px] font-bold text-gray-400 dark:text-white/40">حداکثر</span>
                    <span className="mt-0.5 text-[12px] font-black text-indigo-600 dark:text-indigo-400">{formatNumber(transaction.maximum_stock)}</span>
                </div>
            </div>
            ...
            <div className="relative z-[1] mt-3 flex flex-col gap-2 border-t border-gray-100 pt-2.5 dark:border-white/[0.06]">
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[11px] font-semibold text-gray-600 dark:text-gray-300">{transaction.performed_by_name || "نامشخص"}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                        <CalendarDays size={11} className="text-gray-400" />
                        <span className="text-[11px] font-semibold text-gray-500 dark:text-gray-400">{formatDate(transaction.transaction_date)}</span>
                    </div>
                </div>
                {transaction.note && (
                    <div className="flex items-start gap-1.5 rounded-xl bg-indigo-50 px-2.5 py-2 dark:bg-indigo-500/10">
                        ...
                        <span className="text-[11px] font-medium leading-5 text-indigo-700 dark:text-indigo-300">{transaction.note}</span>
                    </div>
                )}
            </div>
        </motion.div>
...
function ReceiptIcon() {
    return <span className="text-[17px] font-black text-slate-500">#</span>;
}