...
export default function TaskRoutineBoard() {
    ...
    const availableTasks = useMemo(() => {
        const routinedTaskIds = new Set(
            routines.map((routine) => routine.task),
        );

        return tasks.filter(
            (task) => !routinedTaskIds.has(task.id),
        );
    }, [tasks, routines]);
            ...
            {sortedRoutines.length === 0 ? (
                <div className="flex h-64 flex-col items-center justify-center gap-3 rounded-3xl border border-dashed border-gray-200 dark:border-white/[0.07]">
                    ...
                    <button
                        type="button"
                        onClick={() => setCreateOpen(true)}
                        className="rounded-xl bg-indigo-500/10 px-4 py-2 text-[11px] font-bold text-indigo-500"
                    >
                        ایجاد اولین روتین
                    </button>
                </div>
