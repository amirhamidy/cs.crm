...
import { Ban, Check, CheckCircle2, FileUp, Loader, X } from "lucide-react";
import { useTheme } from "next-themes";
import type { InternalTask } from "./Types";
import { cancelInternalTask, completeInternalTask } from "./Api";

interface InternalTaskActionModalProps {
    isOpen: boolean;
    onClose: () => void;
    task: InternalTask;
    action: "complete" | "cancel";
    onDone: (task: InternalTask) => void;
}
...
export default function InternalTaskActionModal({
    ...
    const config = isComplete
        ? {
            title: "تکمیل تیکت",
            subtitle: "در صورت نیاز یادداشت و فایل نهایی را ثبت کن",
            icon: <CheckCircle2 size={15} className="text-emerald-500" />,
            iconBg: "rgba(16,185,129,0.12)",
            buttonGradient: "linear-gradient(135deg, #10b981, #059669)",
            buttonShadow: "0 4px 14px rgba(16,185,129,0.3)",
            submitLabel: "تکمیل تیکت",
            loadingLabel: "در حال ثبت...",
            errorLabel: "خطا در تکمیل تیکت",
        }
        : {
            title: "لغو تیکت",
            subtitle: "در صورت نیاز یادداشت و فایل اضافه کن",
            ...
            submitLabel: "لغو تیکت",
            loadingLabel: "در حال لغو...",
            errorLabel: "خطا در لغو تیکت",
        };
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            const { data } = isComplete
                ? await completeInternalTask(task.id, files, note)
                : await cancelInternalTask(task.id, files, note);
            ...
            >
                ...
                >
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 p-5">
                        <p className="text-[11.5px] font-semibold text-gray-500 dark:text-gray-400">
                            {config.subtitle}
                        </p>
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea
                                ...
                                rows={4}
                                placeholder="یادداشت مربوط به فایل‌ها را بنویس..."
