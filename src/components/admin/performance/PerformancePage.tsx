...
export default function PerformancePage() {
    ...
    const { loading, error, departments, bestEmployees, weakestEmployees, topBringers, topSoldStages, topCancelledStages } = useAnalytics(range);
        ...
        <div className="flex flex-col gap-5 p-3 sm:p-4 md:p-6" dir="rtl">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                        <h1 className="text-[15px] font-extrabold text-gray-900 dark:text-white">آنالیز عملکرد دپارتمان‌ها</h1>
                        <p className="mt-0.5 text-[11.5px] text-gray-400 dark:text-gray-500">برترین‌ها، ریزش فرآیندها و منابع ورودی مشتری</p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                    {chips.map((chip) => (
                        ...
                        >
                            <chip.icon size={11} style={{ color: chip.gradient[0] }} />
                            <span
                                className="bg-clip-text text-transparent"
                                style={{ backgroundImage: `linear-gradient(135deg, ${chip.gradient[0]}, ${chip.gradient[1]})` }}
                            >
                                {chip.value.toLocaleString("fa-IR")}
                            </span>
                            <span className="text-gray-400 dark:text-gray-500">{chip.label}</span>
                        </span>
                    ))}
            ...
            {error && (
                <motion.div
                    ...
                    style={{
                        borderColor: isDark ? "rgba(251,113,133,0.2)" : "rgba(251,113,133,0.15)",
                        background: isDark ? "rgba(251,113,133,0.06)" : "rgba(251,113,133,0.04)",
                        color: "#fb7185",
                    }}
                >
                    {error}
