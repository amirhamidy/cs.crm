...
export const useDepartmentStore = create<DepartmentStore>((set, get) => ({
      ...
      const departments = depts.map((d, i) => {
        ...
        const assignedEmployees = deptAssignments
          .map((a) => {
            ...
            return emp ? mapEmployee(emp, a) : null;
          })
    ...
    return queueMutation(async () => {
      ...
      const maxOrder =
        currentDepartments.length > 0
          ? Math.max(...currentDepartments.map((d) => d.order ?? 0))
          : 0;
      await departmentApi.createDepartment({ name, order: maxOrder + 1 });
    });
  },

  updateDepartment: async (id: string, name: string) => {
    return queueMutation(async () => {
      const dept = get().departments.find((d) => String(d.id) === String(id));
      if (!dept) return;
      ...
      set((state) => ({
        departments: state.departments.map((d) =>
          String(d.id) === String(id) ? { ...d, name } : d,
        ),
        error: null,
      }));
    });
  ...
  updateStage: async (departmentId, stageId, { name, description, order }) => {
    return queueMutation(async () => {
      ...
      const stage = dept?.stages.find((s) => String(s.id) === String(stageId));
          ...
          String(d.id) !== String(departmentId)
            ...
            : {
                ...
                stages: d.stages.filter(
                  (st) => String(st.id) !== String(stageId),
                ),
              },
        ),
        error: null,
  ...
  removeEmployee: async (departmentId: string, employeeId: string) => {
    return queueMutation(async () => {
      ...
      if (!dept || !emp) return;
      if (!emp.assignmentId) throw new Error("شناسه تخصیص یافت نشد");
